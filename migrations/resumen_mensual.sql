DROP FUNCTION IF EXISTS resumen_mensual2;
CREATE OR REPLACE FUNCTION resumen_mensual2(
      ...
      RETURN QUERY
        select *,
          CASE "tabla1"."st_estado"
            WHEN 'PENDIENTE' THEN ("tabla1"."comision_dt" * "PorcentajeOrigen"):: DECIMAL(8, 4)
            WHEN 'PAGADO' THEN ("tabla1"."comision_dt" * "PorcentajeOrigen") :: DECIMAL(8, 4)
            WHEN 'ANULADO' THEN ("tabla1"."comision_dt" * "PorcentajeOrigen") :: DECIMAL(8, 4)
            ELSE 0 :: DECIMAL(8, 4)
          END as "ComisionOrigen",
          CASE "tabla1"."st_estado"
            WHEN 'PENDIENTE' THEN ("tabla1"."comision_dt" * "PorcentajeDestino"):: DECIMAL(8, 4)
            WHEN 'PAGADO' THEN ("tabla1"."comision_dt" * "PorcentajeDestino") :: DECIMAL(8, 4)
            ELSE 0 :: DECIMAL(8, 4) 
        ...
        from (
          ...
          "transferencia"."gastos_administrativos",
          CASE 
          ...
          WHERE 
            (CASE WHEN oficina_origen != '*' THEN "origen"."oficina_codigo" = oficina_origen ELSE true END) 
            AND (CASE WHEN oficina_destino != '*' THEN "destino"."oficina_codigo" = oficina_destino ELSE true END) 
            AND (CASE WHEN oficina_destino != '*' AND oficina_tipo_arreglo = 'REAL' THEN 
                        "transferencia"."op_fecha_hora" between ((DATE(fecha_inicio) + time '00:00:00') AT TIME ZONE 'PET') AND ((DATE(fecha_fin) + time '23:59:59') AT TIME ZONE 'PET') 
                      ...
                      ELSE true 
                END) 
            AND (CASE WHEN oficina_origen != '*' THEN "transferencia"."solicitud_fecha_hora" between ((DATE(fecha_inicio) + time '00:00:00') AT TIME ZONE 'PET') AND ((DATE(fecha_fin) + time '23:59:59') AT TIME ZONE 'PET') ELSE true END) 
		      ORDER BY (CASE  WHEN oficina_origen != '*' THEN  
                            "transferencia"."solicitud_fecha_hora"
                          WHEN oficina_destino != '*' AND oficina_tipo_arreglo = 'REAL' THEN  
                            "transferencia"."op_fecha_hora"
                          WHEN oficina_destino != '*' THEN  
                            "transferencia"."solicitud_fecha_hora"
                          ELSE "transferencia"."solicitud_fecha_hora" 
                    END) ASC 
        ) tabla1;
    END;
  $BODY$;