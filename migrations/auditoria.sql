...
COMMENT ON COLUMN audit.logged_actions.app_user IS 'Usuario de la aplicacion';
...
BEGIN
    ...
    audit_row = ROW(
        ...
        'f',                                          -- statement_only
        NULL                                          -- usuario aplicacion
        );
    

    BEGIN
      IF NEW.usuario IS NOT NULL THEN
        audit_row.app_user = NEW.usuario;
      END IF;
    EXCEPTION WHEN OTHERS THEN
      audit_row.app_user = NULL;
    END;
...
-- CREAR TRIGGERS PARA TODAS LAS TABLAS
 CREATE TRIGGER auditoria_tabla_caja
 AFTER INSERT OR UPDATE OR DELETE ON caja
 FOR EACH ROW EXECUTE PROCEDURE audit.if_modified_func();

 CREATE TRIGGER auditoria_tabla_caja_trabajo
 AFTER INSERT OR UPDATE OR DELETE ON caja_trabajo
 FOR EACH ROW EXECUTE PROCEDURE audit.if_modified_func();

 CREATE TRIGGER auditoria_tabla_centro_costo
 AFTER INSERT OR UPDATE OR DELETE ON centro_costo
 FOR EACH ROW EXECUTE PROCEDURE audit.if_modified_func();

 CREATE TRIGGER auditoria_tabla_cliente_proveedor
 ...
 CREATE TRIGGER auditoria_tabla_documento
 AFTER INSERT OR UPDATE OR DELETE ON documento
 FOR EACH ROW EXECUTE PROCEDURE audit.if_modified_func();

 CREATE TRIGGER auditoria_tabla_documento_serie
 AFTER INSERT OR UPDATE OR DELETE ON documento_serie
 FOR EACH ROW EXECUTE PROCEDURE audit.if_modified_func();

 CREATE TRIGGER auditoria_tabla_empresa
 ...
 CREATE TRIGGER auditoria_tabla_lista_menu
 AFTER INSERT OR UPDATE OR DELETE ON lista_menu
 FOR EACH ROW EXECUTE PROCEDURE audit.if_modified_func();
 ...
 CREATE TRIGGER auditoria_tabla_oficina
 AFTER INSERT OR UPDATE OR DELETE ON oficina
 FOR EACH ROW EXECUTE PROCEDURE audit.if_modified_func();

 CREATE TRIGGER auditoria_tabla_operacion_caja
