      ...
      .then(cajaTrabajo => {
        ...
        if (cajaTrabajo.caja.estado_registro) {
            ...
            if (cajaTrabajo.estado_caja === "ABIERTO") {
              ...
              models.sequelize
                ...
                .then(totales => {
                  ...
                  if (
                    saldo1bd == req.body.Saldo1 &&
                    saldo2bd == req.body.Saldo2 &&
                    saldo3bd == req.body.Saldo3
                  ) {
                    //cerrar caja
                    models.caja_trabajo
                      .update(
                      ...
                      .then(resp => {
                        ...
                        let caja = cache.getValue(cajaTrabajo.caja_codigo);
          ...
          logger.log("error", {
            ubicacion: filename,
            token: token,
            message: { mensaje: "Caja deshabilitada" },
          });
              ...
              if (cajaAnterior) {
                if (cajaAnterior.caja.estado_registro) {
                  if (cajaAnterior.estado_caja === "CERRADO") {
                    ...
                    return models.sequelize
                      .query(
                        `select * from obtener_saldos_caja(:fechaTrabajo, :cajacodigo)`,
                        {
                          replacements: {
                      ...
                      .then(totales => {
                        ...
                        if (
                          ...
                          return models.caja_trabajo
                            ...
                            .then(respuesta => {
                              var socket = req.app.get("socketio");
                              let caja = cache.getValue(cajaAnterior.caja_codigo);
                              caja = JSON.parse(caja);
                              if (caja != null) {
                                caja.estado_caja = "CERRADO";
                                cache.setValue(
                                  cajaAnterior.caja_codigo,
                                  JSON.stringify(caja),
                                );
                                socket.emit(cajaAnterior.caja_codigo + "cerrada", {
                                  mensaje:
                                    "Caja cerrada por usuario " + tokenDecodificado.id,
                                });
                              }
                  ...
                  logger.log("error", {
                    ubicacion: filename,
                    token: token,
                    message: { mensaje: "Caja deshabilitada, no se puede cerrar" },
                  });
                  res
                    .status(409)
                    .send("La caja se encuentra deshabilitada, no se puede cerrar");
                }
              } else {
      ...
      .then(usuario => {
        if (usuario.tipo_arqueo === "VER SALDOS") {
            ...
            .then(cajaTrabajo => {
              ...
              if (cajaTrabajo) {
                ...
                logger.log("warn", {
                  ubicacion: filename,
                  token: token,
                  message: "Caja trabajo no existe",
                });
          ...
          logger.log("warn", {
            ubicacion: filename,
            token: token,
            message: "Usted no puede acceder a esa informacion",
          });
    ...
    .then(cajaTrabajo => {
      if (cajaTrabajo) {
        ...
        logger.log("warn", {
          ubicacion: filename,
          token: token,
          message: "La caja se abrira automaticamente por primera vez",
        });
