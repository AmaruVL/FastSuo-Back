...
//WEB SERVICE PARA LAS PAGINAS DE MONEY EXPRESS Y JUÑUY
exports.buscarGiro = (req, res) => {
  const importe = parseFloat(req.params.importe);
  const dni = req.params.dni;
  axios({
    method: "get",
    baseURL: `https://ww1.essalud.gob.pe/sisep/postulante/postulante/postulante_obtenerDatosPostulante.htm?strDni=${dni}&fbclid=IwAR1kbH5CqTVrjZ7QdAGHqi3ZlPkpfC_lelIhBMFZHrtDcAE7g8fB53pNxtg`,
    url: `/${dni}`
  })
    .then(response => {
      const datos = response.data;
      if (datos.DatosPerson[0].ApellidoPaterno !== "") {
        const nombre_completo = datos.DatosPerson[0].Nombres + " "+ datos.DatosPerson[0].ApellidoPaterno + " " + datos.DatosPerson[0].ApellidoMaterno;
        models.sequelize
        .query(
          `select "St_documento_codigo","St_documento_serie","nro_Solicitud",beneficiario_razon_social,
          solicitante_razon_social,importe, oforigen.oficina_nombre as of_origen, ofdestino.oficina_nombre as of_destino,
          case st_estado when 1 then 'PENDIENTE'
            when 2 then 'PAGADO' when 3 then 'REEMBOLSADO' when 4 then 'ANULADO' end as estado,
          solicitud_fecha_hora from transferencia 
          inner join oficina oforigen on oforigen.oficina_codigo=transferencia.oficina_codigo_origen
          inner join oficina ofdestino on ofdestino.oficina_codigo=transferencia.oficina_codigo_destino 
          where similarity("beneficiario_razon_social", '${nombre_completo}')>0.26 and importe=${importe}
          order by solicitud_fecha_hora desc`,
          {	
